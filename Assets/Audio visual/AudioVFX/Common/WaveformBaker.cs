...
sealed class WaveformBaker : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Lasp.AudioInput.RetrieveWaveform(Lasp.FilterType.Bypass, _array);
        _buffer.SetData(_array, 0, 0, width);

        Lasp.AudioInput.RetrieveWaveform(Lasp.FilterType.LowPass, _array);
        _buffer.SetData(_array, 0, width, width);

        Lasp.AudioInput.RetrieveWaveform(Lasp.FilterType.BandPass, _array);
        _buffer.SetData(_array, 0, width * 2, width);

        Lasp.AudioInput.RetrieveWaveform(Lasp.FilterType.HighPass, _array);
