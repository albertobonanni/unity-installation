...
namespace com.rfilkov.kinect
{
    ...
    public abstract class DepthSensorBase : MonoBehaviour, DepthSensorInterface
    {
        ...
        protected virtual bool UpdatePointCloudColorShader(KinectInterop.SensorData sensorData)
        {
            ...
            if (pointCloudResolution == PointCloudResolution.DepthCameraResolution)
            {
                if (pointCloudAlignedColorTex != null && depthCamColorDataFrame != null && sensorData.lastDepth2ColorFrameTime != lastDepthCamColorFrameTime)
                {
                    lock (depthCamColorFrameLock)
                    {
                        sensorData.lastDepth2ColorFrameTime = lastDepthCamColorFrameTime;
        ...
        protected virtual bool UpdateColorDepthShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamDepthDataFrame != null)
            {
                if(sensorData.usedColorDepthBufferTime == sensorData.lastColorDepthBufferTime && sensorData.lastColorDepthBufferTime != lastColorCamDepthFrameTime)
                {
                    lock(colorCamDepthFrameLock)
                    {
        ...
        protected virtual bool UpdateColorBodyIndexShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamBodyIndexFrame != null)
            {
                if (sensorData.usedColorBodyIndexBufferTime == sensorData.lastColorBodyIndexBufferTime && sensorData.lastColorBodyIndexBufferTime != lastColorCamBodyIndexFrameTime)
                {
                        ...
                        if (bGetBodyData)
                        {
                            ...
                            if (rawBodyIndexImage != null)
                            {
                                ...
                                if (colorCamBodyIndexFrame != null && btCapture != null && btCapture.Color != null && btCapture.Depth != null &&
                                    (rawBodyIndexTimestamp != sensorData.lastBodyIndexFrameTime))
                                {
                            ...
                            //Debug.Log("RawBodyTimestamp: " + rawBodyTimestamp + ", BodyCount: " + trackedBodiesCount + ", QueueLen: " + btQueueCount);
        ...
        public virtual RenderTexture GetDepthCameraColorFrameTexture(ref Texture2D copyToTex2D, ref ulong frameTime)
        {
            if (pointCloudColorTexture == null)
                return null;
            ...
            lock (depthCamColorFrameLock)
            {
                if(copyToTex2D != null && lastDepthCamColorFrameTime != frameTime)
                {
                    KinectInterop.RenderTex2Tex2D(pointCloudColorTexture, ref copyToTex2D);
                    frameTime = lastDepthCamColorFrameTime;
                }
            }

            return pointCloudColorTexture;
        }
        ...
        public virtual ushort[] GetColorCameraDepthFrame(ref ulong frameTime)
        {
            if (colorCamDepthDataFrame == null)
                return null;

            ushort[] frameData = new ushort[colorCamDepthDataFrame.Length];

            lock (colorCamDepthFrameLock)
            {
                KinectInterop.CopyBytes(colorCamDepthDataFrame, sizeof(ushort), frameData, sizeof(ushort));
                frameTime = lastColorCamDepthFrameTime;
            }
        ...
        public virtual byte[] GetColorCameraDepthFrameBytes(ref ulong frameTime)
        {
            if (colorCamDepthDataFrame == null)
                return null;

            byte[] frameData = new byte[colorCamDepthDataFrame.Length * sizeof(ushort)];

            lock (colorCamDepthFrameLock)
            {
                KinectInterop.CopyBytes(colorCamDepthDataFrame, sizeof(ushort), frameData, sizeof(byte));
                frameTime = lastColorCamDepthFrameTime;
            }
        ...
        public virtual byte[] GetColorCameraBodyIndexFrame(ref ulong frameTime)
        {
            if (colorCamBodyIndexFrame == null)
                return null;

            byte[] frameData = new byte[colorCamBodyIndexFrame.Length];

            lock (colorCamDepthFrameLock)
            {
                KinectInterop.CopyBytes(colorCamBodyIndexFrame, sizeof(byte), frameData, sizeof(byte));
                frameTime = lastColorCamBodyIndexFrameTime;
            }
