                        ...
                        if (bBuildMesh)
                        {
                            ...
                            try
                            {
                                ...
                                if (bColorCamRes)
                                {
                                    ...
                                    tDepthImage = sensorData.sensorInterface.GetColorCameraDepthFrame(ref tDepthFrameTime);

                                    ulong tBiFrameTime = 0;
                                    tBiImage = sensorData.sensorInterface.GetColorCameraBodyIndexFrame(ref tBiFrameTime);
                                }
                                ...
                                for (int y = 0, di = 0, ti = 0; y < imageHeight; y++)
                                {
                                    for (int x = 0; x < imageWidth; x++, di++, ti += 6)
                                    {
                                        ...
                                        byte bodyIndex = bColorCamRes ? (tBiImage != null ? tBiImage[di] : (byte)0) : sensorData.bodyIndexImage[di];
                                            ...
                                            if (bUsedVertex || isValidBodyPixel)
                                            {
                                                ...
                                                if (!showAsPointCloud)
                                                {
                                                    ...
                                                    if (isValidBodyPixel && x < imageWidth1 && y < imageHeight1)
                                                    {
                                                        ...
                                                        byte trbi = bColorCamRes ? (tBiImage != null ? tBiImage[tr] : (byte)0) : sensorData.bodyIndexImage[tr];
                                                        byte blbi = bColorCamRes ? (tBiImage != null ? tBiImage[bl] : (byte)0) : sensorData.bodyIndexImage[bl];
                                                        byte brbi = bColorCamRes ? (tBiImage != null ? tBiImage[br] : (byte)0) : sensorData.bodyIndexImage[br];
